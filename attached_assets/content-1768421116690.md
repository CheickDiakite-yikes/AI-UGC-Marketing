...
[Python](https://ai.google.dev/gemini-api/docs/video?example=dialogue#python)More
...
| `resolution` | The video's aspect ratio. | `"720p"` (default), <br>`"1080p"` (only supports 8s duration),<br>`"4k"` (only supports 8s duration)<br>_`"720p"` only for extension_ | `"720p"` (default), <br>`"1080p"` (only supports 8s duration),<br>`"4k"` (only supports 8s duration)<br>_`"720p"` only for extension_ | Unsupported |
