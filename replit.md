...
- **Brand Asset Integration**: AI image generation automatically fetches and includes uploaded brand assets (logos, brand images, and avatar/spokesperson photos) as multimodal input to Gemini. This ensures generated content visually incorporates actual brand elements rather than just text descriptions. Assets are loaded from `profileAssets` table via `brandAssetService.ts`. Avatar images are conditionally included when prompts mention people (using `shouldUseAvatar` detection). The AI system prompt explicitly informs the model that real uploaded images will be passed to image generation, enabling it to reference assets naturally.
